...
import { getCountBySearch } from "@/app/posts/_lib/getCountBySearch";
import { getPostsBySearchInfinite } from "@/app/posts/_lib/getPostsBySearch";

type Props = {
  keyword: string;
};
export default function SearchDashboardWord({ keyword }: Props) {
  const queryClient = useQueryClient();
  const { data: count } = useQuery({
    queryKey: ["posts", "dashboard", "search", "count", keyword],
    queryFn: () => getCountBySearch(keyword),
  });
  const { data, fetchNextPage, hasNextPage, isFetching } = useInfiniteQuery<
    ...
    number
  >({
    queryKey: ["posts", "dashboard", "search", keyword],
    queryFn: ({ pageParam = 0 }) =>
      getPostsBySearchInfinite(pageParam, keyword),
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries({
        queryKey: ["posts", "dashboard", "search"],
        exact: false,
      });
    };
  }, []);
